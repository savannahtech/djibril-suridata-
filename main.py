...
def clean_duplicates(employee_list):
    seen = set()
    cleaned_list = []
    for employee in employee_list:
        employee_tuple = (employee["department"], employee["name"], employee["age"])
        if employee_tuple not in seen:
            seen.add(employee_tuple)
            cleaned_list.append(employee)
    return cleaned_list

def generate_pairs(employee_list):
    dwarfs = [employee for employee in employee_list if employee["department"] == "R&D"]
    giants = [employee for employee in employee_list if employee["department"] == "Sales"]
